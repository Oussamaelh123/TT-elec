  ...
  return (
    <>
      ...
      <div className="real-filters">
        {categories.map(cat => (
          <button key={cat} className={`real-filter${cat === 'Tous' ? ' active' : ''}`}>{cat}</button>
        ))}
      </div>
      ...
      <section className="real-sec">
        <div className="real-grid">
          {realisations.map((r, i) => (
            <div key={r.id} className={`gc rv${i > 0 ? ` d${Math.min(i, 6)}` : ''}${r.feat ? ' feat' : ''}`}>
              <div className="gv">
                <div className="gv-bg" />
                ...
                {r.feat && <span className="gnew">Récent</span>}
              </div>
              <div className="gi">
      ...
      <section className="cta-sec" style={{ padding: '100px 6%' }}>
        ...
        <div className="ovl rv" style={{ justifyContent: 'center', display: 'flex', color: 'rgba(200,146,30,.5)' }}>Votre projet</div>
